...
bestNormalize(score.df$correct_score)
bn_cs <- bestNormalize(score.df$correct_score)
bn_cs_nt <- shapiro.test(bn_cs$x.t)
bn_cs_nt
```

## Kruskal Test Result (SQRT Transformation)

```{r}
result.kruskal_sqrt <- kruskal_test(sqrt(correct_score) ~ Question.Type, data = score.df)
result.kruskal_sqrt
...
## Dunn Test (SQRT Transformation)
```{r}
# Dunn Tests
DT.score <- dunnTest(sqrt(correct_score) ~ Question.Type,
  data = score.df,
  method = "bonferroni"
...
What is changed after sqrt() transformation? 
