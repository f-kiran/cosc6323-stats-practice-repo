...
#Create only 4 levels day2 data
...
## Create boxplot for HR by Day2

```{r echo=FALSE}
all.df2_cut %>%
  filter(Day2 != "NA") %>%
  ggplot(aes(x = Day2, y = HR, fill = Day2)) +
  geom_boxplot() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Boxplot of HR by Day2",
       x = "Day2",
       y = "HR") 
...
all.df.glm = all.df2_cut%>%
  ...
  dplyr::select(HR, Speed_NR, Cadence, Altitude, Distances, Temperatures, Power, Pace2, Period, Age, BMI, max_ind, Day2) %>% 
  drop_na()
...
reduced_data <- subset(all.df.glm)
...
all.df.glm = all.df2_cut%>%
  #filter(Day2 == "MD") %>%
  filter(max_ind != "unknown") %>%
  dplyr::select(ID,HR, Speed_NR, Cadence, Altitude, Distances, Temperatures, Power, Pace2, Period, Age, BMI, max_ind,Day2) %>% 
  drop_na()

all.df.glm <- droplevels(all.df.glm)
...
hr_multi_model <- lmer(HR ~ Speed_NR + Cadence+ Altitude+ Distances +Temperatures + Power +Pace2  + Period + Age + BMI + max_ind +Day2 + (1 | ID), data = all.df.glm)
...
## HR Final Model 
```{r include=FALSE}
step_bw <- step(hr_multi_model, direction = "backward", trace = F)
...
final_hr_multi_model.bw <- lmer(formula = HR ~ Cadence + Distances + Pace2 + Period + Age + BMI + max_ind + Day2 + (1 | ID), data = all.df.glm)

```

```{r echo=FALSE}
#https://rdrr.io/cran/rsq/man/rsq.lmm.html

library(rsq)
summary(final_hr_multi_model.bw)
#display(final_hr_multi_model.bw)
cat("AIC:", AIC(final_hr_multi_model.bw), "\n")
cat("Adjusted R2: \n")
rsq(final_hr_multi_model.bw)
```
...
m.plot <- sjPlot::plot_model(final_hr_multi_model.bw,
                   show.values=TRUE, show.p=TRUE,# show.ci=TRUE, value.offset = 0.5,
                   title="") + 
  theme(plot.title = element_text(hjust = -2, size = 10, face = "bold"),
...
activities2 <- c("office_work","walking", "running", "biking", "driving" )
...
for (i in activities2) {
  ...
  tmp_plot = all.df.glm %>%
          filter(max_ind != i) %>%
          #ggplot(aes(x = Distances, y = HR)) +
          ggplot(aes(x = Pace2, y = HR)) +
          geom_point() + geom_smooth(method='lm', color="red") +
          stat_correlation(mapping = use_label(c("R", "t", "P", "n")),label.x = "right") +
   # scale_x_continuous(breaks = c(0, 10000,200000, 300000)) +
...
final_hr_multi_model.bw <-  
  lmer(formula = HR ~ Cadence  + Pace2 + Period + Age + BMI + max_ind + Day2 + (1 | ID), data = all.df.glm)
...
#tab_model(final_hr_multi_model.bw, title = "Final HR Model")
cat("Adjusted R2: \n")
rsq(final_hr_multi_model.bw)
...
# Interaction between Period and Day2

```{r echo=FALSE}

final_hr_multi_model.bw <-
 lmer(formula = HR ~ Cadence  + Pace2 + Age + BMI + max_ind + Period*Day2 + (1 | ID), data = all.df.glm)
...
## Consolidate physical activities to PA

### max_ind  | Walking + Running+ biking = PA | Reference is office_work 

```{r}

all.df.glm <- all.df.glm %>%
  mutate(max_ind = recode(max_ind, 
...
```{r fig.height=10, fig.width=10, warning=FALSE}
reduced_data <- subset(all.df.glm, select = -c(ID))

#levels(reduced_data$Day2)
...
final_hr_multi_model.bw <-  lmer(formula = HR ~ Cadence  + Pace2 + Age + BMI + max_ind + Period *Day2 + (1 | ID), data = all.df.glm)
...
random_effects <- ranef(final_hr_multi_model.bw) %>%
  pluck(1) %>%
  rownames_to_column() %>%
...
kable_classic(full_width = F, html_font = "Cambria")
  ...
  # kable(align = 'l', booktabs = TRUE, format = "latex", digits = 3) %>%
...
## Random Effects Plot
```{r echo=FALSE}
dotplot(ranef(final_hr_multi_model.bw))
```
